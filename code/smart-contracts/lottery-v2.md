...
**Contract name:** ChangSwapLottery  
...
View [ChangSwapLottery.sol on BkcScan](https://bkcscan.com/address/0x5aF6D33DE2ccEC94efb1bDF8f92Bd58085432d2c#code).

View the [ChangSwap: Lottery contract on BkcScan](https://bkcscan.com/address/0x5aF6D33DE2ccEC94efb1bDF8f92Bd58085432d2c).

## Audits

The ChangSwap Lottery V2 has been audited twice so far. View the results below:

[Peckshield's Lottery V2 Audit](https://github.com/peckshield/publications/blob/master/audit_reports/PeckShield-Audit-Report-ChangswapLottery-v1.0.pdf)
...
```text
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInChang;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] changPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInChang;
        uint32 finalNumber;
```
...
| `priceTicketInChang` | uint256 | The price of a ticket in CHANG \(approximately $5 USD\). |
...
| `changPerBracket` | uint256\[6\] | The amount of CHANG to distribute to winners of each bracket. |
...
| `amountCollectedInChang` | uint256 | The amount of CHANG collected through ticket sales for the Lottery round. |
| `finalNumber` | uint32 | The final number determined by `randomResult` obtained from the number generator contract \([RandomNumberGenerator.sol](https://bkcscan.com/address/0x8c6375Aab6e5B26a30bF241EBBf29AD6e6c503c2)\) using Chainlink VRF. |
...
Calculates rewards for a ticket after draw given the `lotteryId`, `ticketId`, and `bracket`. Filling and querying will provide a link to detailed price information on [BkcScan](https://bkcscan.com/address/0x5aF6D33DE2ccEC94efb1bDF8f92Bd58085432d2c#readContract).
...
Filling and querying will provide a link to detailed price information on BkcScan.

| Name | Type | Description |
| :--- | :--- | :--- |
| `discountDivisor` | uint256 | The divisor for the discount. |
| `priceTickets` | uint256 | The price of a ticket in CHANG. |
...
| :--- | :--- | :--- |
| `lotteryId` | uint256 | The id of the Lottery. |
| `amount` | uint256 | Amount, in CHANG token, to inject. |
...
| `priceTicketInChang` | uint256 | Price of a ticket in CHANG. |
...
Allows admin to recover incorrect tokens sent to address mistakenly. Cannot be CHANG tokens.
...
### setMinAndMaxTicketPriceInChang

```text
function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner;
```

Allows admin to set upper and lower limit of ticket price in CHANG value. Minimum price must be lower than maximum price.

| Name | Type | Description |
| :--- | :--- | :--- |
| `minPriceTicketInChang` | uint256 | The minimum price in CHANG. |
| `maxPriceTicketInChang` | uint256 | The maximum price in CHANG. |
