...
Slippage is not relevant in limit orders. You specify input amount (e.g. 1000 CHANG) and output amount (e.g. 20 KUB), Limit orders guarantee that you will receive no less than the specified output amount (20 KUB) for your input amount (1000 CHANG) if the price for the pair reaches the desired price. **Note that tokens with fee on transfer should not be used with limit orders** (read above)
